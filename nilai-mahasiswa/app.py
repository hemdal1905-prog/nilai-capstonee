...
def init_db():
    with app.app_context():
        db.create_all()
        # Seed contoh mahasiswa jika DB kosong
...
def tambah_mahasiswa():
    ...
    def parse_nilai(key):
        try:
            v = float(request.form.get(key, 0))
    ...
    mhs = Mahasiswa(
        nama=nama, nim=nim,
        clo1=parse_nilai('clo1'), clo2=parse_nilai('clo2'),
        clo3=parse_nilai('clo3'), clo4=parse_nilai('clo4')
    )
    db.session.add(mhs)
    db.session.commit()
...
def edit_mahasiswa(id):
    ...
    mhs.clo1 = parse_nilai('clo1')
    mhs.clo2 = parse_nilai('clo2')
    mhs.clo3 = parse_nilai('clo3')
    mhs.clo4 = parse_nilai('clo4')
    db.session.commit()
...
def dashboard_mahasiswa():
    if session.get('role') != 'mahasiswa':
        return redirect(url_for('login'))
    mhs = Mahasiswa.query.get_or_404(session['mhs_id'])
